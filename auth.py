...
auth_url = preston.get_authorize_url()
print(auth_url)
...
def run_server():
    httpd = HTTPServer(('localhost', 65432), AuthHandler)
    httpd.handle_request()
    return httpd.auth_code

print("Visit the auth URL now and approve access...")
code = run_server()
print("Authorization code received:", code)
auth = preston.authenticate(code)
current_user_info = auth.whoami()
print(current_user_info)
print(auth.refresh_token)
config['refresh_token'] = auth.refresh_token
config_loader.save_auth_config(config)
